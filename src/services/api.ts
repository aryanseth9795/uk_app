...
// export const http = axios.create({
//   baseURL: BASE_URL || 'http://192.168.1.12:3000',
//   timeout: 15000,
// });
...
//   }
//   // fallback: at least equal to selling price so UI doesn't show negative discount
...
// import { normalizeProductsList, mapProductLite, type ProductLite } from '@utils/productMapper';
...
//   const { data } = await http.post(routes.auth.login, body);
//   return data;
// }
// export async function apiRegister(body: {
//   name: string; email: string; phone: string; password: string; confirm_password: string;
// }) {
//   const { data } = await http.post(routes.auth.register, body);
//   return data;
// }
// export async function apiRefresh(refresh_token: string) {
//   const { data } = await http.post(routes.auth.refresh, { refresh_token });
//   return data;
// }
// export async function apiMe() {
//   const { data } = await http.get(routes.auth.me);
//   return data;
// }

// // ---- Products (normalized) ----
// export async function apiGetProductsLite(params?: { page?: number; per_page?: number }): Promise<ProductLite[]> {
//   const { data } = await http.get(routes.products(), { params });
//   return normalizeProductsList(data);
// }

// export async function apiSearchProductsLite(q: string): Promise<ProductLite[]> {
//   const { data } = await http.get(routes.search(q));
//   return normalizeProductsList(data);
// }

// export async function apiGetProductLite(id: string): Promise<ProductLite> {
//   const { data } = await http.get(routes.product(id));
//   return mapProductLite(data);
// }

// // ---- Suggest (labels only) ----
// export async function apiGetSuggestions(q: string): Promise<string[]> {
//   const { data } = await http.get(routes.suggest(q));
//   if (Array.isArray(data) && data.every((s) => typeof s === 'string')) return data as string[];
//   if (Array.isArray(data)) {
//     const out: string[] = [];
...
/* --------------- PRODUCTS ----------------- */
/** Normalized list for grid */
export async function apiGetProductsLite(params?: { page?: number; per_page?: number }): Promise<ProductLite[]> {
  const { data } = await http.get(routes.products(), { params });
  return normalizeProductsList(data);
}

/** Normalized list for search results */
export async function apiSearchProductsLite(q: string): Promise<ProductLite[]> {
  const { data } = await http.get(routes.search(q));
  return normalizeProductsList(data);
}

/** Normalized single product (minimal fields for cards) */
export async function apiGetProductLite(id: string): Promise<ProductLite> {
  const { data } = await http.get(routes.product(id));
  return mapProductLite(data);
}

/** FULL product detail (images, variants, etc.) */
export async function apiGetProductDetail(id: string): Promise<any> {
  const { data } = await http.get(routes.product(id));
  return data; // return raw; your detail screen can use images/variants/etc.
}

/** 🔧 Back-compat alias — many screens call this name */
export const apiGetProductById = apiGetProductDetail;
